...
export class IndicatorsController {
	...
	@ApiHeader({
		...
		description: 'Token de autenticación interno (configurado en SSM Parameter Store)',
		required: true,
	})
	@ApiResponse({
		status: 201,
	...
	ingestTestRun(@Body() dto: LambdaIngestionDto, @Headers('x-internal-token') token: string) {
		if (token !== process.env.INTERNAL_TOKEN) {
			console.warn('Unauthorized access attempt with token:', process.env.INTERNAL_TOKEN);
	...
	@ApiQuery({
		name: 'projectId',
		description: 'ID único del proyecto del cual se desean obtener los indicadores',
		required: true,
	})
	@ApiResponse({
		status: 200,
