...
import { IsString, IsEnum, IsISO8601, IsNumber, IsOptional } from 'class-validator';
...
export class LambdaIngestionDto {
	...
	@ApiPropertyOptional({
		description: 'Vulnerabilidades de severidad BAJA detectadas (security - menor prioridad)',
		example: 10,
	})
	@IsOptional()
	@IsNumber()
	...
	@ApiPropertyOptional({
		description: 'Vulnerabilidades informativas (security - solo información, no requieren acción)',
		example: 3,
	})
	@IsOptional()
	@IsNumber()
	...
	@ApiPropertyOptional({
		description: 'Total de peticiones HTTP realizadas en la prueba de carga (performance)',
		example: 1000,
	})
	@IsOptional()
	@IsNumber()
