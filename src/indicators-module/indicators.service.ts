...
export class IndicatorsService {
	...
	async createFromLambda(dto: LambdaIngestionDto) {
		...
		const projects = await this.dynamodb.queryGSI('GSI2', `PRODUCT#${dto.teamName}`);
		...
		if (dto.pipelineType === 'regression') {
			const totalTests =
				dto.totalTests ?? (dto.passed ?? 0) + (dto.failed ?? 0) + (dto.skipped ?? 0);
			const passed = dto.passed ?? 0;
			const failed = dto.failed ?? 0;
			const skipped = dto.skipped ?? 0;

			const executionSuccessRate = this.metricsCalculator.calculateExecutionSuccessRate(
				passed,
				totalTests
			);
			const automationCoverage = this.metricsCalculator.calculateAutomationCoverage(
				totalTests,
			...
			const weightedScore = high * 10 + medium * 5 + low * 2 + informational * 1;
			const securityScore = totalVulnerabilities === 0 ? 100 : Math.max(0, 100 - weightedScore);
	...
	async deleteAllByProject(projectId: string): Promise<number> {
		...
		if (indicators.length > 0) {
			...
			const deleteRequests = indicators.map((indicator: Record<string, any>) => ({
				DeleteRequest: {
