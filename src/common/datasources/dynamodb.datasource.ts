...
export class DynamoDBDatasource {
	...
	constructor() {
		this.tableName = process.env.DYNAMODB_TABLE_NAME || 'kata-backend-production';
		this.createInstance();
	}
	...
	async get(pk: string, sk: string): Promise<any> {
		try {
			...
			const result: GetCommandOutput = await this.docClient.send(new GetCommand(params));
	...
	async query(
		...
		try {
			...
			const result: QueryCommandOutput = await this.docClient.send(new QueryCommand(params));
			return result.Items || [];
		} catch (error: unknown) {
			const err = error as Error;
	...
	async scan(options?: {
		...
		try {
			...
			const result: ScanCommandOutput = await this.docClient.send(new ScanCommand(params));
	...
	async update(
		...
		try {
			...
			const result: UpdateCommandOutput = await this.docClient.send(new UpdateCommand(params));
