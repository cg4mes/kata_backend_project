...
export class UsersService {
	...
	async login(loginDto: LoginDto): Promise<LoginResponseDto> {
		...
		const users = await this.dynamodb.queryGSI('GSI1', `EMAIL#${loginDto.email}`);
		...
		const isPasswordValid = await this.validatePassword(loginDto.password, user.password as string);
		if (!isPasswordValid) {
			throw new UnauthorizedException(ERROR_MESSAGES.INVALID_CREDENTIALS);
		}
	...
	async findAll(): Promise<UserResponseDto[]> {
		...
		return items.map((user: Record<string, any>) => this.toUserResponse(user));
	}
	 ...
	 * @throws NotFoundException if user doesn't exist
	 */
	async updateRole(id: string, updateUserRoleDto: UpdateUserRoleDto): Promise<UserResponseDto> {
		const user = await this.findUserById(id);
		...
		this.logger.log(`User role updated: ${user.email as string} -> ${updateUserRoleDto.role}`);
	...
	private async validateUniqueUser(email: string, username: string): Promise<void> {
		// Check username (PK)
		// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
		const userByUsername = await this.dynamodb.get(`USER#${username}`, 'METADATA');
		if (userByUsername) {
